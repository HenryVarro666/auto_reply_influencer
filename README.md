...
1. **① fetch 抓帖** —— 对 CSV 里每个账号，用 Instagram 公开的 `web_profile_info` 接口（HTTP/2 + 轮换代理，**无需登录**）拉取最近的帖子，只保留**过去 N 小时**内发布的，下载配图并入库。重复运行**不会重复保存**同一条帖子（按 `post_id` 用 `INSERT OR IGNORE` 去重）。
...
cp .env.example .env        # 然后把你的代理填进 .env 的 IG_PROXY（若已提供则已填好）
```
...
| 未上传 | 是什么 | 怎么得到 |
|---|---|---|
| `.env` | 真实密钥（`IG_PROXY` 代理、各家 API Key） | `cp .env.example .env` 后填入；代理向上游负责人索取 |
...
cp .env.example .env                     # 2. 填 .env：至少填好 IG_PROXY
...
- **抓帖报大量限流 / 失败？** 确认 `.env` 里 `IG_PROXY` 已填好；不带代理抓 100 个账号几乎必被限流，可先 `--limit` 小批量测试。
